...
_ks = ks_utils = {
  ...
  /*  5. 对象 S */
    inObj : function (param,obj){  
      for(var i in obj){  
        if('undefined' !== typeof obj[i] && obj.hasOwnProperty(i) && param===obj[i]) return true;  
      }  
      return false;  
    },
    extend : function(){
      ...
      for (var i = 0,aLen = args.length; i < aLen; i++) {
        args[i] =args[i] || {};
        for (var j in args[i]) {
          if(args[i].hasOwnProperty(j) && 'undefined' !== typeof args[i][j] ){
            obj[j] = args[i][j];
          }
        }
      }
    ...
    "date" : {
      ...
      */
      ...
      "getLastTime": function(time) {
        ...
        if (1 > diff) str = "刚刚";
        else if (6e4 > diff) str = parseInt(diff / 1e3) + "秒前";
        else if (18e5 > diff) str = parseInt(diff / 6e4) + "分钟前";
